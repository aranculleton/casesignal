# TODO

## Current focus: CaseSignal MVP

- [x] structured data foundation (snapshots -> events -> labels -> training slice)
- [x] AI integration plan (`docs/ai_workflow_roadmap.md`)

- [ ] phase 1: baseline model script (`scripts/train_baseline_v1.py`)
- [ ] phase 2: synthetic servicing-note generator (`scripts/generate_case_notes_v1.py`)
- [ ] phase 3: servicing-note signal extraction module (`scripts/extract_note_signals_v1.py`)
- [ ] phase 4: hybrid scoring module (`scripts/build_hybrid_score_v1.py`)
- [ ] phase 5: case summary + action output (`scripts/generate_case_summary_v1.py`)
- [ ] phase 6: baseline-vs-hybrid evaluation + model-card note
